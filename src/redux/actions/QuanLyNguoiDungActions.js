...
export const themNguoiDungAction = (formData) => {
  return async (dispatch) => {
    try {
      await quanLyNguoiDungService.themNguoiDung(formData);
...
export const layThongTinLoaiNguoiDungAction = () => {
  return async (dispatch) => {
    try {
      // Sử dụng tham số thamSo
      const result =
        await quanLyNguoiDungService.layThongTinLoaiNguoiDungAction();
      // Sau khi lấy dữ liệu từ api về => redux (reducer)
      dispatch({
...
export const capNhatThongTinNguoiDungAction = (FormData) => {
  return async (dispatch) => {
    try {
      await quanLyNguoiDungService.capNhatThongTinNguoiDung(FormData);
